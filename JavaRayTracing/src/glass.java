public class glass extends Material{
    ...
    public boolean scatter(Ray r, HitRecord rec, Wrapper wrapper) {
        ...
        if (r.dir().dot(rec.normal) > 0) {
            ...
            ni_over_nt = ref_idx;
            cosine = r.dir().dot(rec.normal) / ( r.dir().length() * rec.normal.length() );      //入射角余弦
        }
        else {
            // 球->空气
        ...
        if (refract(r.dir(), outward_normal, ni_over_nt, wrapper)) {
            //发生了折射 计算反射系数
        ...
        if (Math.random() < reflect_prob) {
            wrapper.scatter = new Ray(rec.p, reflected);
        }
        else {
            wrapper.scatter = new Ray(rec.p, wrapper.refracted);
        }
        return true;
    }
     ...
     * @return 是否有折射
     */
    public boolean refract(vector v, vector n, float nt, Wrapper wrapper){
